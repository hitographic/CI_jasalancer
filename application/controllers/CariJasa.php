...
class CariJasa extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['judul'] = 'Jasa - JasaLancer | Temukan Freelancer Terbaikmu di sini ';
        ...
        $config['base_url'] = site_url('CariJasa/index'); //site url
        ...
        $data['jasa'] = $this->Jasa_model->get_jasa_list($config["per_page"], $data['page'], $data['keyword']);
  
        $data['pagination'] = $this->pagination->create_links();
  
        //load view mahasiswa view
        $this->load->view('templates-depan/header', $data);
        $this->load->view('CariJasa/index', $data);
    ...
    {
        $this->load->model('Jasa_model');
        $data['jasaku'] = $this->Jasa_model->getAllCariJasa($yes)->row_array();
        $data['user'] = $this->db->get_where('user', ['email' => $this->session->userdata('email')])->row_array();
        $data['judul'] = 'Detail Data Freelancer';
   
        
        $this->load->view('templates-depan/header', $data);
        $this->load->view('CariJasa/detail', $data);
        $this->load->view('templates-depan/footer');
    }
}